...
export default {
  ...
  setup() {
    ...
    const connect = (scanID) => {
      ...
      const options = {
        ...
        clientId: "web_" + "client_id_" + Math.floor(Math.random() * 1000000 + 1),
        username: '',
        password: '',
      };
      ...
      const connectUrl = "wss://" + options.host + ":" + options.port + options.endpoint;
      ...
      client.value.on('connect', () => {
        ...
        client.value.subscribe(`r2GqN8hAWxyFoGh/scanners/${scanID}/#`, { qos: 0 }, (error) => {
          if (error) {
            console.error('Error al suscribirse:', error);
          } else {
            console.log('Suscripción exitosa al tema r2GqN8hAWxyFoGh/scanners/${scanID}/#');
            client.value.publish(`r2GqN8hAWxyFoGh/scanners/${scanID}/input`, "status");
      ...
      client.value.on('message', (topic, message) => {
        ...
        if (topic === `r2GqN8hAWxyFoGh/scanners/${scanID}/btn`) {
                ...
                //   } else {
                    ...
                    // }
                    // if (this.tab === "total") {
                      if (mmScanning.value < 300 || mmScanning.value > 780) {
                        $q.notify({
                          message: mmScanning.value < 300 ? "cm min 40" : "cm max 78",
                          color: "negative",
                          multiLine: true,
                          timeout: 3500,
                          position: "center",
        ...
        if (topic === `r2GqN8hAWxyFoGh/scanners/${scanID}/avg`) {
                ...
                if (message.toString() === "btnOff") {
                  $q.notify({
                    message: "Has soltado el botón antes de tiempo. La medición ha sido cancelada.",
                    color: "info",
                    multiLine: true,
                    timeout: 2500,
                    position: "center",
        ...
        if (topic === `r2GqN8hAWxyFoGh/scanners/${scanID}/output`) {
          ...
          mmScanning.value = ((Math.PI * wheelDiameter) / 80) * parseInt(dataMqtt.value[dataMqtt.value.length - 1][0]);
        }
    ...
    const sendMessage = () => {
      if (client.value && isConnected.value) {
        client.value.publish('tema/ejemplo', messageToSend.value, { qos: 0 }, (error) => {
          if (error) {
  ...
  function getShowScannerStatusBanner() {
    console.log("getShowScannerStatusBanner-NewTotalScan");
      const userUID = $q.localStorage.getItem('userUID')
